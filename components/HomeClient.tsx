    ...
    <div className="space-y-16 py-4">
      ...
      <section className="glass-card-accent p-8 lg:p-14 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl space-y-6">
          <div className="inline-flex items-center gap-2 bg-slate-100 border border-slate-300 dark:bg-zinc-800 dark:border-zinc-700 px-3.5 py-1.5 rounded-full text-xs font-semibold text-slate-800 dark:text-zinc-200">
            <Sparkles className="w-3.5 h-3.5 text-slate-900 dark:text-white" />
            {t('hero.badge')}
          </div>
          <h1 className="text-4xl lg:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-tight">
            {t('hero.title1')} <span className="mono-gradient-text">{t('hero.titleHighlight')}</span>
          </h1>
          <p className="text-slate-600 dark:text-zinc-300 text-base lg:text-lg leading-relaxed">
            {t('hero.description')}
          </p>
          ...
          {/* Quick Stats Bar */}
          <div className="grid grid-cols-3 gap-6 border-t border-slate-200 dark:border-zinc-700/80 pt-8 mt-8 text-slate-700 dark:text-zinc-300">
            <div>
              <span className="block text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white">{t('hero.stat1Num')}</span>
              <span className="text-xs text-slate-500 dark:text-zinc-400">{t('hero.stat1Text')}</span>
            </div>
            <div>
              <span className="block text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white">{t('hero.stat2Num')}</span>
              <span className="text-xs text-slate-500 dark:text-zinc-400">{t('hero.stat2Text')}</span>
            </div>
            <div>
              <span className="block text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white">{t('hero.stat3Num')}</span>
              <span className="text-xs text-slate-500 dark:text-zinc-400">{t('hero.stat3Text')}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-8">
        <div className="flex items-end justify-between border-b border-slate-200 dark:border-zinc-800 pb-4">
          <div>
            <h2 className="text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{t('featured.title')}</h2>
            <p className="text-slate-500 dark:text-zinc-400 text-sm mt-1">{t('featured.subtitle')}</p>
          </div>
          <Link href="/catalog" className="text-slate-900 dark:text-white hover:underline text-sm font-semibold flex items-center gap-1">
          ...
          {featuredPlans.map((plan) => (
            <div key={plan.id} className="glass-card overflow-hidden group flex flex-col justify-between">
              <div className="relative aspect-[16/10] overflow-hidden bg-slate-100 dark:bg-zinc-900">
                ...
                <div className="absolute top-3 left-3 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md px-3 py-1 rounded-lg border border-slate-200 dark:border-zinc-700 text-xs font-semibold text-slate-900 dark:text-zinc-200">
                  {plan.style}
                </div>
                <div className="absolute bottom-3 right-3 bg-slate-900 dark:bg-white text-white dark:text-zinc-950 font-black px-3 py-1 rounded-lg text-sm shadow-md">
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-slate-700 dark:text-zinc-300 bg-slate-50 dark:bg-zinc-900/80 p-3 rounded-xl border border-slate-200 dark:border-zinc-700/70 font-mono">
      ...
      <section className="glass-card-accent p-8 lg:p-10 border-l-4 border-l-slate-900 dark:border-l-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2 max-w-2xl">
          <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-sm">
            <ShieldCheck className="w-5 h-5" />
            {t('callout.badge')}
          </div>
          <h3 className="text-xl font-bold text-slate-900 dark:text-white">{t('callout.title')}</h3>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-6 space-y-3">
          <FileCheck className="w-8 h-8 text-slate-900 dark:text-white" />
        ...
        <div className="glass-card p-6 space-y-3">
          <Layers className="w-8 h-8 text-slate-900 dark:text-white" />
