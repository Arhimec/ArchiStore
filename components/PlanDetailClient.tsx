...
export default function PlanDetailClient({ plan }: { plan: Plan }) {
  ...
  return (
    <div className="space-y-8 sm:space-y-12">
      ...
      <div className="space-y-3 border-b border-slate-200 dark:border-zinc-800 pb-5 sm:pb-6">
        <div className="flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs font-mono text-slate-500 dark:text-zinc-400">
          <span>{t('pdp.catalog')}</span> / <span className="text-slate-900 dark:text-white">{plan.style.toUpperCase()}</span> / <span className="text-slate-700 dark:text-zinc-200 truncate max-w-[160px] sm:max-w-none">{plan.title}</span>
        </div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">{plan.title}</h1>
            <p className="text-slate-600 dark:text-zinc-400 text-xs sm:text-sm mt-1 leading-relaxed">{plan.description}</p>
          </div>
          <div className="bg-slate-100 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 px-5 py-3 rounded-2xl flex md:flex-col items-center justify-between md:items-end">
            <span className="block text-[11px] sm:text-xs font-semibold text-slate-500 dark:text-zinc-400 uppercase">{t('pdp.licensePrice')}</span>
            <span className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white font-mono">${plan.price.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="glass-card p-3 sm:p-4 space-y-4">
            {/* Gallery Tabs */}
            <div className="flex items-center gap-2 sm:gap-4 border-b border-slate-200 dark:border-zinc-800 pb-3">
              <button
                onClick={() => { setActiveTab('renders'); setActiveImageIndex(0); }}
                className={`text-xs font-bold px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl transition-all ${
                  activeTab === 'renders'
            ...
            {/* Main Preview Frame */}
            <div className="relative aspect-[4/3] sm:aspect-[16/10] bg-slate-100 dark:bg-zinc-900 rounded-xl overflow-hidden border border-slate-200 dark:border-zinc-800">
              {currentImage ? (
                <img
                  src={
                ...
                <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-zinc-600 font-mono text-xs">
                  No image preview available
                </div>
              )}

              {(currentImage?.isFloorPlan || activeTab === 'floorplans') && (
                <div className="absolute top-2 right-2 sm:top-3 sm:right-3 bg-slate-900/90 dark:bg-zinc-900/90 border border-slate-700 dark:border-zinc-700 text-white px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[10px] sm:text-[11px] font-mono font-bold flex items-center gap-1.5 shadow-lg">
              ...
              <div className="flex items-center gap-2.5 overflow-x-auto pb-2">
                {displayedImages.map((img, idx) => (
                  <button
                    ...
                    className={`relative w-16 h-12 sm:w-20 sm:h-14 rounded-lg overflow-hidden border transition-all flex-shrink-0 ${
          ...
          <div className="glass-card-accent p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <a
              ...
              className="btn-secondary text-xs px-4 py-2.5 w-full sm:w-auto text-center flex items-center justify-center gap-2 shrink-0"
            >
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="glass-card-accent p-5 sm:p-6 space-y-6 lg:sticky lg:top-24">
            ...
            <form onSubmit={handleCheckout} className="space-y-5 sm:space-y-6">
              ...
              <div className="bg-slate-50 dark:bg-zinc-900 p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-zinc-700 space-y-3">
